...
# returns a new array of the hashes sorted by the sum values of the block
# applied to the corresponding keys in the hashes.

# You may assume that the input array will only contain hashes, and that the
# hashes will have unique keys.
...
puts sort_by_key([{a: 2, b: 3}, {a: 1, b: 2}, {a: 3, b: 4}]) { |x| x }
puts sort_by_key([{a: 1, b: 2}, {a: 3, b: 4}]) { |x| -x }
